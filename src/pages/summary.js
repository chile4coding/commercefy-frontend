...
import { getInvoice, getNotification, setCurrentInvoice, setGenereatedInvoices, setUser } from '@/redux/storeSlice';
import { generatePaymentLink, getCookie, getInvoices, getUser, socket } from '@/services/request';
...
dispatch(setGenereatedInvoices(data.updateInvoice));
 ...
 const datN = await getInvoices(token);

 dispatch(getInvoice(datN.invoices));

   const response = await getUser(token);
      const datUser = await response.json();

      if(response.status === 200){
        dispatch(setUser(datUser.owner));
  ...
  return (
    <Applayout>
      ...
      <div className="text-center p-2 mb-5">
        <button
          onClick={preview}
          className="text-[#FEFEFE] btn  bg-[#4F378B] px-16 py-3 rounded-md border hover:bg-[#4F378B]  hover:text-white">
